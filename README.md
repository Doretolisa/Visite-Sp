# 📘 Visite SP

O **Visite SP** é um site desenvolvido para apresentar e divulgar os principais pontos turísticos da cidade de **São Paulo (SP)**.  
O objetivo é oferecer aos visitantes — tanto moradores quanto turistas — uma maneira simples, visual e informativa de conhecer locais icônicos, históricos e culturais da maior cidade do Brasil.

🔗 **Acesse o site:**https://visitesp.vercel.app/**

---

## 🚀 Tecnologias Utilizadas

- **HTML5**
- **CSS3**
- **JavaScript**

---
## 📝 Sobre o Projeto

O **Guia Turístico de São Paulo** foi desenvolvido como um site interativo que permite aos usuários explorar diversos pontos turísticos da cidade de forma rápida e intuitiva.  
A proposta do projeto é facilitar a busca por lugares interessantes para visitar, oferecendo descrições, imagens e informações úteis sobre cada destino.

O site funciona como um catálogo digital, onde qualquer pessoa — seja morador da cidade ou turista — pode pesquisar locais famosos, históricos, culturais e de lazer. A navegação simples e visual torna a experiência agradável, ajudando o usuário a descobrir novos espaços e planejar passeios pela cidade de São Paulo.

Além disso, o projeto foi pensado para ser leve, responsivo e de fácil acesso, permitindo que todos encontrem informações de forma prática, sem a necessidade de downloads ou cadastros.


---

## 📥 Instalação

```bash
# Clone este repositório
git clone https://github.com/SEU-USUARIO/SEU-REPOSITORIO.git

# Acesse a pasta do projeto
cd SEU-REPOSITORIO

# Abra o arquivo principal
start index.html
```

---

## 🌆 O que você encontra no site

- 🏙️ Informações sobre pontos turísticos famosos  
- 🖼️ Fotos e descrições detalhadas  
- 🗺️ Locais históricos, culturais e arquitetônicos  
- 🚶 Roteiros ideais para quem quer conhecer SP  
- 📱 Site acessível e fácil de navegar  

---

## 👩‍💻 Autora

Monalisa Doreto Alencar
📧 Email: doretomonalisa@gmail.com

---

## 📝 Licença

Este projeto é de uso aberto para fins educacionais.

---

## 🙏 Créditos

Este projeto foi inspirado na necessidade de facilitar o acesso a informações turísticas da cidade de São Paulo.
